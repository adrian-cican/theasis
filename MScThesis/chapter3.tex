...
Supervised learning is the most widely adopted ML paradigm and is used when a dataset contains both the input variables, commonly referred to as \textit{features}, \textit{covariates}, or \textit{predictors}, and 
...
In recent years, self-supervised learning has become one of the most active research areas in ML, achieving remarkable success in CV, NLP and speech processing.

\subsection{Tree-Based Ensemble Models} \label{sec:tree_ensembles}

Not every learning problem requires deep architectures. For tabular data, where observations are described by a
moderate number of heterogeneous features, traditional ML algorithms remain highly competitive and are frequently
adopted as reference baselines. Among these, tree-based ensembles are the most widely used, and two of them,
Random Forest and XGBoost, are employed in this dissertation as baselines against which the DL models are compared.

The building block of these methods is the \emph{decision tree}, which recursively partitions the feature space
into increasingly homogeneous regions. At each node, the algorithm selects the feature and the threshold that best
separate the observations according to a purity criterion, and a prediction is obtained by following the resulting
sequence of decisions until a leaf is reached. Individual trees are attractive because they are easy to interpret,
require no feature scaling, and handle features of different natures without preprocessing. However, a single
sufficiently deep tree tends to fit the noise of the training data, resulting in high variance and poor
generalisation (\cite{Hastie2009}).

Ensemble methods address this limitation by combining a large number of trees, following one of two complementary
strategies. In \emph{bagging}, many trees are grown independently on different random samples of the training data
and their predictions are averaged, which reduces the variance of the ensemble. In \emph{boosting}, trees are
instead grown sequentially, each one trained to correct the errors made by the ensemble built so far, which
progressively reduces the bias (\cite{Hastie2009}).

\emph{Random Forest} (\cite{Breiman2001}) is the most established bagging method. In addition to training each
tree on a bootstrap sample of the data, it restricts each split to a random subset of the available features. This
additional source of randomness decorrelates the individual trees, so that averaging their predictions yields a
substantially larger reduction in variance than bagging alone. Random Forests are robust to noise, require little
hyperparameter tuning, and provide a natural measure of feature importance, which makes them a common first choice
for tabular problems.

\emph{XGBoost} (\cite{Chen2016}) is the most widely adopted boosting method. Each new tree is fitted to the
gradient of the loss function with respect to the current predictions of the ensemble, so that successive trees
concentrate on the observations that remain poorly predicted. To this formulation XGBoost adds explicit
regularisation of the tree complexity, which limits overfitting, together with a set of algorithmic optimisations
that allow it to scale efficiently to large datasets. These properties have made it one of the best-performing
algorithms for tabular data and a standard baseline in applied ML.

Despite their effectiveness, both methods operate on fixed-length feature vectors and have no intrinsic notion of
temporal ordering. When applied to time-series data, the sequence of observations must therefore be either
flattened into a single vector or summarised through hand-crafted aggregate statistics, in both cases discarding
part of the temporal structure of the signal. This limitation is precisely what motivates the recurrent
architectures introduced in Section~\ref{sec:dparqui}, and is the reason why these models are adopted here as
baselines rather than as the primary approach.

\subsection{Representation Learning} \label{sec:rep_l}

The ML paradigms presented in the previous section describe the different ways in which models can learn from data. Regardless of the learning 
paradigm, the performance of a ML algorithm largely depends on the quality of the data representation used to describe the problem.
...
Once the architecture of a neural network has been defined, the next step is to
determine the values of its parameters, namely the weights and biases. This
process, known as training, consists of adjusting those parameters so as to
minimise a \emph{cost function}, a scalar measure of how far the predictions of
the model are, on average, from the correct answers over the training set
(\cite{Goodfellow2016}).

The search is carried out iteratively by \emph{gradient descent}. At any given
setting of the parameters, the gradient of the cost indicates how sensitive the
error is to a change in each individual parameter, and therefore in which
direction each one should move in order to reduce it. Every parameter is then
nudged a small amount in that direction. The size of the step is controlled by
the \emph{learning rate}, a hyperparameter that must be chosen with care: steps
that are too large overshoot the minimum and cause the training to diverge, while
steps that are too small make convergence impractically slow. The gradients
themselves are obtained through \emph{backpropagation}, which applies the chain
rule of calculus to propagate the error backwards through the network, layer by
layer, so that the sensitivity of every parameter is recovered in a single
backward pass (\cite{Rumelhart1986}).

Computing the cost over the entire training set before each update would be
prohibitively expensive, so the gradient is instead estimated from a small,
randomly drawn subset of examples called a \emph{mini-batch}. This variant,
\emph{stochastic gradient descent}, gives a noisier estimate of the gradient but
allows far more updates for the same computational budget, and the noise itself
can help the optimiser escape poor solutions (\cite{Goodfellow2016}). Rather than
applying one common learning rate to every parameter, modern optimisers adapt the
step size individually, using the history of previous gradients. The most widely
used is \emph{Adam} (\cite{Kingma2015}), which converges faster than plain
stochastic gradient descent and is less sensitive to the initial learning rate,
and is therefore the optimiser adopted for all models trained in this work.

Each iteration thus comprises four stages: the mini-batch is passed through the
network to obtain predictions, the cost is evaluated, the gradients are computed
by backpropagation, and the parameters are updated. A complete pass over the
training set is called an \emph{epoch}, and training proceeds over many epochs
until the model converges or a predefined stopping criterion is reached, as
summarised in Figure~\ref{fig:training_pipeline}.
...
\node[box] (input)                        {Sample mini-batch from training set};
\node[box, below=of input]    (forward)   {Forward propagation};
\node[box, below=of forward]  (loss)      {Cost computation};
\node[box, below=of loss]     (backward)  {Backpropagation};
\node[box, below=of backward] (update)    {Parameter update (SGD / Adam)};
...
\draw[arrow] (update.east) -- ++(1.2,0) coordinate (turn) |- (input.east);
\node[font=\footnotesize, anchor=west, align=left] at (turn)
      {next mini-batch\\(until convergence)};

\end{tikzpicture}

\caption[Training pipeline of a deep neural network]{Training pipeline of a deep
neural network. Each iteration processes one mini-batch through the four stages;
backpropagation computes the gradients, which the optimiser then uses to update
the parameters. A complete pass over the training set constitutes one epoch.}
...
AD, FD, and PdM. Although these concepts are closely related, they address different stages of the 
...
In industrial environments, AD plays a fundamental role in condition monitoring by continuously analyzing sensor data to identify deviations 
...
Unlike FD, AD does not necessarily identify the cause or type of the abnormal behaviour. Instead, its primary objective is to determine 
...
Depending on the availability of labelled data, AD can be formulated either as a supervised binary classification problem, distinguishing between normal and 
anomalous observations, or as an unsupervised task, in which the model learns the distribution of normal operating conditions and flags significant 
...
operation (\cite{Isermann2006}). In contrast to AD, which only indicates that abnormal behaviour is present, FD aims to identify the 
...
multiclass classification problem, in which each observation is assigned to one of several predefined fault categories, or to a no-fault class representing 
...
Although AD, FD, and PdM are closely related, they address different stages of the equipment health monitoring 
process. AD focuses on identifying abnormal behaviour without necessarily determining its cause. Once an anomaly has been detected, FD 
aims to identify the existence and, when possible, the type of fault responsible for the abnormal behaviour. PdM extends this process by 
...
shared structure is what motivates the MTL approach adopted in this dissertation, in which a single model exploits a shared representation of 
...
RUL of a component, or as a classification problem, predicting the likelihood of 
...
with task-specific output branches, consistent with the MTL strategy introduced in 
...
in isolation, whether AD, FD, or RUL estimation, each typically supported by a dedicated DL model 
(\cite{Khan2026,Zaidi2026}). Although DL has achieved remarkable performance on these individual problems, comparatively few works have investigated 
MTL strategies capable of jointly addressing multiple monitoring objectives within a single model. The multi-task approaches that do exist are 
generally limited to combining two of these tasks, such as AD and RUL estimation (\cite{Yin2026}), or health-state assessment 
and RUL estimation (\cite{Yan2023}), rather than jointly addressing AD, FD, and PdM within a 
unified framework.

Furthermore, the application of these DL and MTL approaches to biomethane upgrading plants remains, to the best of our knowledge, 
...
These observations motivate the development and evaluation of a multi-task DL framework capable of jointly performing AD, FD, 
and PdM for the vacuum pump of a biomethane upgrading plant, and of comparing its performance against both single-task DL models and traditional ML baselines.